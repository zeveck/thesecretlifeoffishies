...
1. **Game source changes** (`js/**`):
   - Find the corresponding test file(s) in `tests/unit/`
...
3. **Pre-existing failure protocol.** If a test fails in code you didn't
   ...
   b. **Research and file a GitHub issue** (`gh issue create`) with:
      ...
      - Reproduction command: `npx vitest run tests/unit/<file>.test.js`
...
1. **Run `npm test` again** — all suites must pass, including new tests
...
2. **Detail cards** — only for items with `[ ]` in User column:
   ...
   ![animation](.playwright/output/1-fish-animation.png)
   ```
