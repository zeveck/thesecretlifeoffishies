...
- **focus** (optional) -- prioritize a specific domain:
  - `new` -- recently filed issues, user feedback
  - `behavior` -- fish behavior and AI correctness
  - `rendering` -- visual rendering bugs
  - `genetics` -- genetics and breeding system
  - `ui` -- UI and interaction bugs
...
- `/fix-issues 10 behavior` -- interactive, fish behavior focus, run now
...
|---|-------|---------|----------|
| #12 | Fish overlap bug | FIXED | fish.js:85, commit abc1234, 2 tests |
| #15 | Breeding crash | FIXED | genetics.js updated, 3 tests pass |
| #20 | Some bug | LIKELY FIXED | Code changed but no regression test |

Close 2 FIXED issues? (all / comma-separated numbers / none)
```
...
1. **Scan `SPRINT_REPORT.md`** for skipped issue numbers across all sprints,
   deduplicate against existing plans in `plans/`, and check GitHub issue state.
...
3. **Present findings** (unless `auto`):
   ...
   > | # | Title | Source |
   > |---|-------|--------|
   > | #42 | Complex fish AI | Sprint 2026-03-17: Skipped -- Too Complex |
   > | #63 | Genetics overhaul | Sprint 2026-03-16: Remaining Open |
   > ...
...
4. **Update issue trackers** -- ensure tracker files in `plans/` reflect
   current GitHub state. Create tracker files as needed (e.g.,
   `plans/ISSUES_PLAN.md`, `plans/QE_ISSUES.md`).
...
2. **Fetch the research blurb from plan files:**
   ```bash
   grep -A 30 '#<N>' plans/*.md 2>/dev/null
   ```
...
2. Correctness defects (fish behavior, genetics, simulation)
...
- **Too complex** -- clear spec but would require 500+ lines, major
  ...
  - If `plans/` contains plan files, grep them for the issue number
...
- **Same root cause** -- if #100 and #101 are both caused by an off-by-one
  in fish.js, group them for the same agent. One agent, one worktree,
  one fix closes both.
- **Same file** -- if #200 and #201 both need changes to genetics.js, group
...
- **Unrelated issues get separate agents.** Never batch unrelated hard
  issues into one agent -- this caused bottlenecks when one agent
  got diverse issues sequentially.
...
2. **The research blurb from the plan file** (tracker files in `plans/`).
...
6. **Agent verification** -- if UI files changed, use playwright-cli with
   real events, take screenshots as evidence. Verify the fix visually.
7. **Classify User Verify** -- if visual or UI files changed, mark
   `User Verify: NEEDED` in the sprint report. The user must see UI
   changes before the issue can be closed.
...
new `## Sprint` section. `/fix-report` marks sections
...
|---|-------|----------|--------|-------|-------------|-------------|
| #123 | Fish overlap | wt-123 | abc1234 | 2 unit | PASS (tests) | N/A |
...
**User Verify:** Does the user need to see this? If visual or UI files
changed -> `NEEDED`. Otherwise -> `N/A`. `/fix-report` Step 2 presents
all `NEEDED` items for user review before closing.
...
|---|-------|-----------------|
| #789 | Render error | cherry-pick conflict on commit abc1234 |
  ...
  1. **Try cherry-picking WITHOUT stashing first.** Git allows cherry-picks
     ...
     **If git refuses** with "your local changes would be overwritten,"
     ...
     2. **Capture the pre-stash state** of each overlapping file:
        `git diff <file>` -- save/remember this output.
...
(also destroys untracked files). The conflicted state preserves all data --
leave it as-is and report to the user:
...
Create the file if it doesn't exist. Add a `## Sprint Failed` section:
...
- `npm test` fails after cherry-picks are landed (Phase 6 step 8)
...
- **`npm test` before every commit** -- all test suites must pass.
