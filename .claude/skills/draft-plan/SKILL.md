...
- **description** (required) — everything after the recognized keywords.
  Can be brief ("add dark mode") or detailed ("implement a genetics system
  with dominant/recessive traits, inheritance, and mutation").
...
- `/draft-plan GENETICS_PLAN.md Implement genetics system` → writes `plans/GENETICS_PLAN.md`
- `/draft-plan plans/GENETICS_PLAN.md Implement genetics system` → same
- `/draft-plan output plans/GENETICS_PLAN.md rounds 5 Implement genetics system`
- `/draft-plan rounds 5 Implement genetics system with trait inheritance`
...
2. **Patterns agent** — read existing plans in `plans/` for format and style
   reference. Read `CLAUDE.md` for constraints. Identify conventions and
   rules that the plan must respect.

3. **Domain agent** — research the specific technical domain. For game
   mechanics: understand the simulation model, fish behavior, genetics,
   rendering. For UI work: understand the interaction model, existing
...
output filename if one was provided (e.g., `GENETICS_PLAN`
→ `/tmp/draft-plan-research-GENETICS_PLAN.md`). If no output
...
- Phases would be vague ("implement fish genetics") because the breadth
...
Past failure: a large plan covered too much — phases were
...
- [ ] Criterion 2 — e.g., "test that fish with two recessive alleles display the recessive trait"
...
- **Be specific, not vague.** "Implement the Guppy class with color genetics
  using Mendelian inheritance for body color" — not "implement genetics."
...
- **Acceptance criteria must be testable.** "Works correctly" is not testable.
  "Test that offspring of two heterozygous parents produce 25% homozygous
  recessive phenotype over 100 trials" is testable.
...
'implement the genetics system' but doesn't specify the allele representation,
dominance rules, or mutation rates — the agent will have to guess all three"
is actionable.
...
4. **Present the result:**
   ...
   >
   > Written to `plans/GENETICS_PLAN.md` — open it up and let me know
   > what to change.
   >
   > Execute with: `/run-plan plans/GENETICS_PLAN.md`
   > Or with scheduling: `/run-plan plans/GENETICS_PLAN.md auto every 4h now`
