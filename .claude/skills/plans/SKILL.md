...
3. If the file exists, display a **actionable dashboard** — not a one-line
   ...
   Ready to Run:
     RENDERING_GAPS_PLAN.md           9 gaps     High
     GENETICS_PLAN.md                 4 phases   Medium
     AQUARIUM_FEATURES_PLAN.md        5 phases   Medium

   In Progress:
     BEHAVIOR_PLAN.md                 Phase 4b   4/8 done
     PERFORMANCE_PLAN.md              Phase 1    1/3 done

   Needs Review: 3 plans (old format, status ambiguous)

   Next: /run-plan plans/RENDERING_GAPS_PLAN.md
   ```
...
4. If the file is older than 24 hours (check mtime), append:
   > Warning: Index is older than 24 hours. Run `/plans rebuild` to refresh.
5. **Exit.**
...
3. Display grouped by status (Ready, In Progress, Complete), with the
   ...
   ```
   Ready to Run:
     RENDERING_GAPS_PLAN.md (5 phases, Medium)
       Fix rendering gaps: missing animations, stale sprite counts, broken effects

     GENETICS_EXPANSION_PLAN.md (4 phases, Medium)
       Add 15 new heritable traits via phased genetics system updates

   In Progress:
     PERFORMANCE_PLAN.md (13 phases, 7/13 done)
       Systematic frame rate improvements with benchmark reference tests

   Complete:
     AUDIO_PLAN.md (7 phases)
       Background music and sound effects system with 5 modes
     ...
   ```
...
Get all plan files. Ignore subdirectories.
...
1. **Recently created plans** — sort by git creation date (newest first).
...
2. **Alphabetical** — tiebreaker.

Assign priority labels: **High** (newest / most urgent), **Medium**
...
|------|--------|------------|----------|-------|
| [EXAMPLE_PLAN.md](EXAMPLE_PLAN.md) | 5 | 1 -- Setup | High | Recently created |

## In Progress

| Plan | Phases | Current Phase | Next Phase | Notes |
|------|--------|---------------|------------|-------|
| [BEHAVIOR_PLAN.md](BEHAVIOR_PLAN.md) | 8 | 4b -- Schooling | 4c -- Predation | 4 of 8 phases done |
...
|------|--------|-------|-------|
| [REFACTOR_PLAN.md](REFACTOR_PLAN.md) | 3 | No progress tracker | Check if refactor was implemented |

## Complete

| Plan | Phases | Notes |
|------|--------|-------|
| [GENETICS_META.md](GENETICS_META.md) | 4 | Meta-plan — all sub-plans done |
|   -> [TRAIT_SYSTEM_PLAN.md](TRAIT_SYSTEM_PLAN.md) | 3 | Sub-plan of GENETICS_META |
|   -> [BREEDING_PLAN.md](BREEDING_PLAN.md) | 2 | Sub-plan of GENETICS_META |
| [AUDIO_PLAN.md](AUDIO_PLAN.md) | 3 | All phases done |
...
- **Meta-plan grouping:** If a plan is a meta-plan (has `delegate /run-plan`
  ...
  This makes the hierarchy visible.
