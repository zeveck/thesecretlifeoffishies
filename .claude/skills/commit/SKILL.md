...
- `/commit fish genetics fix push` — scope-guided commit + push
...
- `/commit fish behavior fixes` → scope: "fish behavior fixes", action: commit
...
- `/commit genetics fixes land` → scope: "genetics fixes", action: land
...
1. **Keyword grep** — search the file list for scope-related terms:
   ...
   For `/commit fish behavior`, grep for `fish`, `behavior`, `js/`.
...
2. **Run tests if code was staged** — if any staged files are code (`.js`,
   `.css`, `.html`), run `npm test` before committing. All
...
6. **Write `.landed` marker** on the worktree (so cleanup knows
   it's safe to remove):
   ```bash
