...
- **area** (optional, with bash) -- specific feature or area to bash. If
  ...
  recent changes. Examples: `"fish behavior"`, `"genetics/breeding"`,
...
- Check that test files in `tests/unit/` and `tests/e2e/` cover new functionality
...
1. **Select target area:**
   - If area specified (e.g., `bash "fish behavior"`): use that area
...
7. **Clean up test files:**
   ...
   - Run `npm test` before committing -- all suites must pass
