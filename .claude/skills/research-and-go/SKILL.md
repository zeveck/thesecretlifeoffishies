...
Examples:
- `/research-and-go Add a genetics system with trait inheritance and mutations`
- `/research-and-go Implement all missing fish species from the design doc`
- `/research-and-go Add multiplayer tank visiting with live sync`
...
- **This is the top of the pipeline.** The execution chain:
  `/research-and-go` → `/research-and-plan` → `/draft-plan` (xN) →
  `/run-plan` (meta) → `/run-plan` (xN sub-plans) → `/verify-changes`
  (xN) → `/commit` (xN landings).
