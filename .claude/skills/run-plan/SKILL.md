...
- **plan-file** (required) — path to plan, e.g. `plans/FISHIES_PLAN.md`
- **phase** (optional) — specific phase, e.g. `4b`. If omitted, auto-detect
...
Examples:
- `/run-plan plans/FISHIES_PLAN.md` — interactive, next phase
- `/run-plan plans/FISHIES_PLAN.md 4b` — interactive, specific phase
- `/run-plan plans/FISHIES_PLAN.md finish` — interactive, all remaining phases (pauses between each)
- `/run-plan plans/FISHIES_PLAN.md finish auto` — autonomous, all remaining phases (no pausing)
- `/run-plan plans/FISHIES_PLAN.md auto every 4h` — schedule every 4h
- `/run-plan plans/FISHIES_PLAN.md auto every 4h now` — schedule + run now
- `/run-plan plans/FISHIES_PLAN.md status` — show plan progress
...
4. Present a progress table:
   ...
   ```
   Plan: plans/FISHIES_PLAN.md

   | Phase | Status |
   |-------|--------|
   | 1 — Fish Base Class | Done (abc1234) |
   | 2 — Genetics System | Done (def5678) |
   | 3 — Breeding Logic | Next ← |
   | 4 — UI Integration | Blocked (needs 3) |

   Next phase: 3 — Breeding Logic
   Dependencies: 1 ✓, 2 ✓
   ```
...
3. Report:
   - If found: parse the cron expression and compute the next fire time.
     ...
     > Prompt: Run /run-plan plans/FISHIES_PLAN.md auto every 4h
...
2. **Extract phases and status** — handle four formats:
   - **Progress tracker table**: rows with `✅ Done`,
...
7. **Save the VERBATIM phase text** — copy the entire section from the plan
   ...
   **Do NOT summarize, paraphrase, or reinterpret.** The plan is the spec.
   ...
   Lesson: summarized descriptions cause agents to implement the wrong thing.
   A vague title like "reset button" was interpreted as "clear canvas"
   ...
   instead of passing verbatim text.
...
8. **Classify UI impact from the plan text.** Scan the phase description
   ...
   reference to `style.css`, `index.html`, or UI-related JS files.
...
2. **If ANY phase was classified as UI-touching** (step 8), dispatch a
...
- **`### Execution: delegate <skill> [args]`** — delegate mode. The phase
  runs a skill (e.g., `/run-plan`) that manages its own
...
One worktree for the entire phase (not per-item).
...
2. **Agent prompt MUST include the verbatim plan text.** The implementing
   ...
   summary, not bullet points extracted from it. The full section with every
   requirement, formula, constraint, design note, and acceptance criterion.
...
3. **If dispatching sub-agents for parallel work items**, each sub-agent gets:
   ...
   - Their **specific scope** clearly delineated
   - **What parallel agents are doing** — enough to avoid conflicts (shared
     ...
     their scope.
   - **Shared infrastructure dependencies** — if a base class or shared
     module must exist first, that must be built sequentially before
...
4. **Within-phase parallelism is the agent's judgment call** — if items are
   independent, the agent may dispatch sub-agents. If there's shared
   infrastructure to build first, it works sequentially then parallelizes.
   The skill does NOT force parallelism.
...
6. **Running tests in worktrees — CRITICAL.** Agents waste hours getting
   ...
   > 1. Run tests with output captured to a file:
   ...
   > 2. If tests fail, **read `.test-results.txt`** to find the failures.
   ...
   >    when the single file takes seconds.
   > 3. After fixing, run the single file again to confirm. Then run
   >    `npm test > .test-results.txt 2>&1` ONE more time as
   >    the final gate before committing.
   > 4. Max 2 fix attempts at the same error — do not thrash.
   > 5. If a test fails in code you didn't touch, it may be pre-existing.
...
7. **No steps skipped or deferred.** If the plan says "implement 7 components,"
   implement 7 components. If it says "write tests for breeding logic," write
...
2. **Run `npm test` on main** — the delegate already tested, but
...
1. **Dispatch verification agent** targeting the worktree's changes. The
   ...
   - The **worktree path** from Phase 2 (so it can read files and run tests
     there via `cd <worktree-path> && npm test`)
...
2. **Additional plan-specific checks** (the verifier checks these against the
   ...
   - Does implementation follow the plan's stated approach?
...
3. **If verification fails:**
   ...
   - With `auto`: dispatch fix agent targeting the worktree path (NOT with
     `isolation: "worktree"` — that would create a new worktree). Give it the
     worktree path, verbatim plan text, and verification findings. After the
     fix agent finishes, re-verify (max 2 rounds). If still failing after
     2 fix+verify cycles, **STOP** — needs human judgment. Invoke the
     Failure Protocol.
...
1. **Update the plan file's progress tracker on main** — change the phase
   ...
   landed commits) and notes. Examples by format:
   - Table: `| **2: Genetics** | ✅ Done | \`abc1234\` | trait system, 12 tests |`
   - Checklist: `- [x] Phase 2 — Genetics System (abc1234, trait inheritance)`
...
is derived from the plan filename (e.g., `FISHIES_PLAN.md` → `plan-fishies`).
...
Past failure: a report was overwritten each run, losing previous data.
Same applies here — never overwrite.

**Report format** — each phase appends a section:

```markdown
## Phase — 2 Genetics System [UNFINALIZED]

**Plan:** plans/FISHIES_PLAN.md
**Status:** Completed (verified)
**Worktree:** ../plan-fishies-2
**Commits:** abc1234, def5678

### Work Items
| # | Item | Status | Commit |
|---|------|--------|--------|
| 1 | Allele representation | Done | abc1234 |
| 2 | Trait inheritance | Done | def5678 |
...
1. Open the app, observe the fish tank
2. Click on a fish to view its details
3. Verify the genetics panel shows trait information
4. Check that offspring display inherited traits correctly

![genetics panel](.playwright/output/phase2-genetics-panel.png)
```

**Conditional:** Only include the User Verification section if UI files
(`style.css`, `index.html`, or UI-related JS files) were changed in this
phase. If no UI files changed, omit the entire section.
...
- **With `auto` and NO User Verify items:** Auto-land verified phase
  ...
  4. **If a cherry-pick conflicts:** unlike individual issues, a plan phase
     is one logical unit — partial landing is not useful. Abort all
     cherry-picks and invoke the **Failure Protocol**.
     ```bash
...
- `npm test` fails after cherry-picks are landed
